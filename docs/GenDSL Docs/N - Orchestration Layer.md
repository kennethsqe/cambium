...
- **CLI**: `cambium run app/pipelines/<name>.pipeline.rb --method <method> --arg <input>` mirrors gens. **Every declared `input` slot is mandatory** — the declaration takes no `optional:` and no `default:`, so a single-slot pipeline binds the raw `--arg` string and a multi-slot pipeline requires a JSON object supplying *every* slot. A parseable object that leaves any slot unbound raises, naming the missing ones (#226); the check is own-property only, because `constructor` is a legal slot name. This lives in the binder, so it holds for `cambium serve`, `cambium replay` and library callers, not just the CLI. Genfile catalog (used by `cambium serve`) recognizes pipelines alongside gens.
...
**Nothing shared, nothing warmed** (RED-183): when the cacheable branches split into singleton groups there is no race to win, so the prewarm steps aside instead of paying one cold write per branch. A heterogeneous fan-out — every reviewer with its own system prompt or its own source — used to buy N caches that exactly one branch would read, one tick later. The gate is all-or-nothing: a fan-out that mixes shared and unshared branches still warms its singleton groups.
...
- **Best-effort** — a warm-up failure never fails the fan-out; that group's branches just run cold. Outcomes surface in the `PipelineFanOut` trace `meta.prewarm`, including a skip (`meta.prewarm.skipped`) and a `[cambium] prewarm skipped:` line on stderr.
- **On by default; opt out with `prewarm false`.** Fan-out-level gates (entire prewarm skipped): `concurrency 1` (branch 1 warms the rest), single-branch fan-outs, `--mock` runs, `prewarm false`, and nothing-shared — every cacheable branch in its own group. Per-branch filter (inside `prewarmFanOut`): ungrounded branches, branches whose cacheable prefix is below the Anthropic cache floor (`MIN_CACHE_PREFIX_CHARS`), and branches whose sub-gen is `mode :agentic` (#228) are excluded from the warm-up group; the prewarm still runs for the remaining eligible branches, so `meta.prewarm.groups` can be zero even when `meta.prewarm` is present. The nothing-shared gate compares `groups` against **eligible** (cacheable) branches, not `branches.length` — excluded branches are not evidence of a shared prefix, and 5 branches with 3 excluded and 2 distinct groups still has nothing to warm. Every one of those exclusions happens **before** the `eligible` counter increments, for that reason.

**Why agentic branches are excluded** (#228, DEC-005): prewarm warms through `generateText`, which sends no tools, and Anthropic builds cache prefixes `tools` → `system` → `messages`, each level on the last. A tools-less warm-up therefore diverges from the real agentic call at the *first* level of the hierarchy, so the entry it writes could not be read even in principle. Teaching prewarm to send the tools array too is a follow-up, not part of #228.
