...
- **Explicit `appRoot`/`engineDir` precedence for shipped IR (#195, 2026-09-03).** This section's discovery order is source-anchored (walk up from `ir.entry.source`, or from cwd as a fallback). A precompiled artifact (`cambium serve --precompiled`/`--ir-dir`, `cambium run --ir`) breaks that assumption on purpose: `entry.source` is a build-machine path that may not exist — or may exist but point at an unrelated tree — on the machine executing the shipped artifact, while its own on-disk location is always the right anchor. **Operator contract, extended:** `runGenFromIr`'s `opts.appRoot` now wins as the tier-1 workspace outright whenever `<appRoot>/Genfile.toml` exists (previously it only anchored tool/action/provider/log-sink discovery, per the touchpoint table above, while contracts + correctors still fell through to the source-anchored walk-up — RED-393 left that seam half-open). `opts.engineDir` gets the same explicit-wins treatment. Both callers that pass either option (serve's precompiled dispatch, `cambium run --ir`) compute it from the artifact's own path, never from `entry.source`; every other existing caller (`cambium run`, pipeline sub-gen dispatch) is unaffected — `opts.appRoot`/`opts.engineDir` are `undefined` unless explicitly passed, so the new tier-1 check is a no-op for them and behavior is byte-identical. See [`N - Precompiled IR Distribution (#195)`](N%20-%20Precompiled%20IR%20Distribution%20%28%23195%29.md) for the full picture.
...
4. **Schema co-location wiring.** *(Shipped in RED-243, extended RED-274.)*
   ...
   - Extended (RED-274): the CLI's `main()` now resolves schemas from the app's `Genfile.toml` `[types].contracts` list when present — `packages/cambium-runner/src/genfile.ts` reads the file, validates each path (rejects absolute paths, `..` escapes, null bytes), imports each via `pathToFileURL`, and merges named exports into the schemas registry. The Genfile lookup is **source-anchored**: `findGenfileDir(ir.entry.source)` walks up from the gen file looking for the nearest `Genfile.toml`, and only falls back to cwd when the source has no Genfile ancestor. This matches the engine-mode and ModelAliases stance — the gen's package, not the host's cwd, is the source of truth. (A bare cwd lookup misresolved when a Cambium app ran with cwd inside a different Cambium workspace; the engine-mode lookup right above this one already anchored on `entry.source`, this one now does too.) When no `Genfile.toml` is found via the walk-up or at cwd (or it has no `[types].contracts`, e.g. a `[workspace]` Genfile), the CLI falls back to importing `packages/cambium/src/contracts.ts` — preserving back-compat for the monorepo's own example app. Surfaced while building the first external dogfood; fixes the prior "paste your schema into the framework repo" workaround.
   - **Genfile-without-`[types]` fix (#205 A-002/A-003, 2026-09-03).** The RED-274 fallback above conflated three distinct situations under one `resolveGenfileContracts(...) === null` check: no `Genfile.toml` at all, a Genfile with no `[types]` section, and an empty `[types].contracts` list. All three fell through to the monorepo-only `packages/cambium/src/contracts.ts` import — for a flat `[package]` workspace whose gens use 100% inline `returns do … end` schemas (RED-419's normal shape for an app that never needed a contracts.ts), this crashed `cambium run` with `ERR_MODULE_NOT_FOUND` before Generate ever ran, even though `contractsMod` is never read when `ir.returnSchema` is inline. **Operator contract, corrected:** `runGenFromIr` now branches on whether **a Genfile reached by walking up from the gen's source** was found (`findGenfileDir(ir.entry.source)`, source-anchored — the same lookup this section already describes), not on whether it declared contracts, and not on whether cwd merely happens to contain an unrelated Genfile. Three tiers, in order: (1) the gen's own workspace, when found this way — contracts if it declares `[types]`, `{}` if it doesn't (a `[types]`-less Genfile is app mode with no contracts, e.g. a fresh `[package]` app whose gens are 100% inline; a symbol-form gen in that shape now fails at the unchanged "Schema not found" site naming `[types].contracts`, not a module-resolution crash); (2) else, cwd's own declared `[types]` contracts, if cwd's Genfile has them (the pre-existing RED-220 host-compiled-IR / container case, unchanged); (3) else, the monorepo cwd fallback, unchanged. App correctors (RED-275) are discovered from the gen's own workspace at tier 1, or cwd's at tier 2, either way. A-002's first cut mistakenly treated *any* Genfile discoverable at cwd (tier 1, not tier 2) as the gen's own — which broke nine tests that spawn the CLI from the monorepo root against a gen living in an unrelated tmpdir, since the monorepo's own root `[workspace]` Genfile (no `[types]`, by design) was wrongly read as "this gen's workspace declares no contracts." A-003 restricted tier 1 to source-anchored discovery only; a Genfile merely sitting at cwd is tier 2, and pre-A-002 behavior there is untouched. Residual, deliberately unfixed: a stray gen file outside any workspace, run from an app root whose own Genfile has no `[types]`, still takes the monorepo fallback and fails with `ERR_MODULE_NOT_FOUND` exactly as on `main` — that layout has no realistic user. `resolveGenfileContracts` itself is unchanged throughout; its `null` return is still correct for "no contracts list to read," it's just interpreted per-tier now instead of collapsing "no Genfile" and "Genfile with no `[types]`" into one case.
...
6. **Engine-mode runtime catch-up.** *(Landed 2026-04-20; see git log for commits and the merged PR.)*
...
7. **Engine-mode authoring polish.** *(Landed 2026-04-20, same day as item 6; see git log.)*
   ...
   - Resolved:
     - `cli/lint.mjs` — new `lintEngine(engineDir)` walks siblings with the same regex + JSON-shape checks app-mode uses. `runLint()` does sentinel-first detection so lint run from inside an engine folder operates on that folder, not the host workspace. Validates gen cross-references (`returns`, `system`, `uses`, `corrects`, `security`, `budget`, memory `scope:`, `action`) against siblings. Reserved scope names (`:session`, `:global`) are skipped from pool lookup.
     - `vscode/cambium-syntax/src/server.js` — `scanWorkspace` now finds engine folders via `findEngineFolders()` (workspaceRoot + `cambium/*` + `packages/*`) and layers sibling scans on top of any app-mode scan. Engine siblings win on name collision. Extension bumped 0.5.0 → 0.6.0.
     - `cli/generate.mjs` — `generateSchema` in engine mode now appends exports directly to `<engineDir>/schemas.ts` (creating the file with a typebox import when absent; idempotent on duplicate name via a single `export const <Name>` regex, later hardened — superseded by #206, item 9 below). App-mode behavior originally unchanged (superseded by #158 — see item 8 below) — `src/contracts.ts` was treated as a shared workspace artifact that a scaffolder shouldn't quietly edit.
     - `cli/cambium.mjs` — `cambium run` emits a stderr note when the gen's sibling `<name>.ir.json` has mtime older than the `.cmb.rb`. The hint fires only in engine mode; `cambium run` itself recompiles and is unaffected by stale IR — the risk is host code using the typed wrapper that imports the committed IR directly.
     - `packages/cambium/tests/engine_mode_live_e2e.test.ts` (new) — opt-in live-LLM end-to-end test (skipped unless `CAMBIUM_LIVE_TESTS=1`). Catches drift in real-model response handling the `--mock` path can't.
   - 13 new lint/scaffolder tests, 1 new opt-in live test. Full suite: 706 passed | 2 skipped.

8. **App-mode `generateSchema` now writes, fail-closed (issue #158, 2026-09-01).**
   - Context: item 7 above recorded app-mode `generateSchema` as print-only, on the stated principle that `src/contracts.ts` is a shared workspace artifact a scaffolder shouldn't quietly edit. That stance held until `cambium new pipeline` (RED-374) started declaring `input :document, schema: <Name>Input` — a required kwarg with no default — which left every fresh pipeline scaffold unable to compile until the developer manually added the schema by hand, the one "compiles out of the box" property `cambium new agent` already had.
   - Resolved: `generateSchema`'s app-mode branch now appends directly to `src/contracts.ts` — create the file with the TypeBox import when absent (via the shared `writeFile` overwrite-protection helper), skip with an "exists (skipped)" message when the name is already present, append otherwise. `generatePipeline` invokes it for `<Name>Input` before writing the pipeline file, so a fresh `cambium new pipeline` now compiles immediately.
   - Fail-closed existing-name detection: the first cut of this write path checked only a literal `export const <Name>` regex, which missed other export idioms a hand-authored `contracts.ts` might use (e.g. `const X = …; export { X }`) and silently appended a second, colliding declaration — a real corruption bug caught by adversarial audit, not by construction. Hardened: the name now counts as already-present — skip, never append — when ANY of a literal `export const <Name>`, an `export { … <Name> … }` list anywhere in the file, or a top-level `const|let|var|class|function|type|interface|enum <Name>` declaration matches. Skip is the fail-safe direction — a false "exists" costs the developer one export added by hand; a false "absent" corrupts their file. **At the time this landed, the engine-mode branch above still ran the weaker single-pattern check from item 7 — the two branches disagreed on what "already present" means (closed by #206, item 9 below).**
   - This supersedes the "App-mode behavior unchanged" line in item 7 above: app mode writes now, under the same two guards (fail-closed name detection, shared overwrite-protection helper) rather than staying print-only.
   - Tests: `packages/cambium/tests/scaffolder_schema_app_mode.test.ts` (fresh file, literal-export skip, export-list-idiom skip, pipeline-invoked skip) and `packages/cambium/tests/scaffolder_pipeline_shape_aware.test.ts` (pipeline scaffold compiles out of the box, idempotent on re-run).

9. **Engine-mode `generateSchema` gets the same fail-closed detection app mode already had (issue #206, 2026-09-03).**
   - Context: item 8's fail-closed three-pattern detection was app-mode-only — engine-mode's `generateSchema` (item 7) still ran the original single-pattern `export const <Name>` check, so the two branches disagreed on what "already exported" means. A hand-authored `schemas.ts` exporting via `const X = …; export { X }` was invisible to the engine-mode check, and `cambium new schema X` silently appended a second, colliding `const`/`$id` declaration — the same corruption class item 8 closed for app mode, reopened one branch over.
   - Resolved: the three-pattern detection is now factored into a single helper, `schemaNameAlreadyPresent(fileContents, pascalName)`, shared by both branches — engine-mode's `schemas.ts` append and app-mode's `contracts.ts` append call the same function. App mode's observable behavior is byte-identical (same three patterns, same order, same skip/append messages); only engine mode's detection changed. As of this change the two branches are not merely "the same idiom" (as item 8 claimed) but literally the same function — the drift item 8 introduced is closed.
   - The shared helper also escapes its `pascalName` argument before interpolating into the three `new RegExp(...)` calls (`AUD-206-05`) — a no-op for every name `validateName`'s `/^[A-Za-z][A-Za-z0-9_]*$/` gate allows through today, but the helper no longer trusts that precondition silently now that it has two callers.
   - Tests: `packages/cambium/tests/scaffolder_schema_engine_mode.test.ts` (new — fresh file, literal-export skip, and the `export { X }`-idiom repro that mirrors item 8's app-mode test) plus `scaffolder_schema_app_mode.test.ts` (unmodified, still green — the byte-identical proof).
...
1. **CLI cwd-relative paths.** `cli/compile.mjs` (and `cli/cambium.mjs` for `run`) hardcoded `./ruby/cambium/compile.rb`. Fine inside the monorepo, broken from a host project. Fix: resolve the Ruby script relative to `import.meta.url` of the CLI module. Applied to `cli/compile.mjs`; `cambium run`'s same gap is not blocking (engine-mode hosts call `runGen` directly) and remains as future work. **Resolved in RED-306**: `cambium run` now invokes `runGenFromIr` in-process (Option B), the prior subprocess is eliminated, and the CLI's Ruby-script path is `import.meta.url`-relative like `compile.mjs`. **Extended in RED-376**: `cli/serve.mjs` had the same gap — `compile.rb` was located via an up-4-levels constant inside `@redwood-labs/cambium-runner` that pointed at `node_modules/` rather than the sister `@redwood-labs/cambium` package. Fixed symmetrically by computing the path in `cli/serve.mjs` relative to `import.meta.url` and threading it through a new `runServe({ compileRb })` option; see [[C - Serve Mode]] § "Locating `compile.rb`". **Closed in RED-380**: `packages/cambium-runner/src/memory/retro-agent.ts` was the last cwd-relative spawn site — `invokeRetroAgent` ran `spawnSync('node', ['cli/cambium.mjs', …])`, so a `write_memory_via :Agent` declaration silently dropped its writes from any non-monorepo cwd (engine-mode host, npm-installed `[package]` app). Fixed with the same precedence chain as the others: `resolveDefaultCli(override)` resolves `opts.cambiumCli` → `CAMBIUM_CLI` env → `createRequire(import.meta.url).resolve('@redwood-labs/cambium/package.json')` → in-tree monorepo fallback. With this, no spawn site in the codebase depends on `process.cwd()` being the monorepo root.
...
- [[N - Precompiled IR Distribution (#195)]] — the shipped-IR anchoring rule added to item 4, in full.
