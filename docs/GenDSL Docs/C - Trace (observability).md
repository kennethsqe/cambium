...
| `parent_run_id` | Replayed runs only (RED-312) | The run id this run was resumed from (`cambium replay <run-id>`). Absent on first-class runs. Chains compose: `run_A → run_B(parent=A) → run_C(parent=B)`, so the full replay lineage is walkable. See [[P - cambium replay]]. |
| `replay` | Replayed Pipeline runs only (RED-385 Phase B) | `{ from_op, reused_op_count }` — the operator execution resumed at, and how many upstream operators were reused (rehydrated from the parent trace rather than re-dispatched). Reused operator entries in `operators[]` carry `reused: true`. |
...
| `LogEmitted` | A log primitive emission (`log :destination`) dispatched successfully to one destination (RED-282 / RED-302). One step per destination per event. `ok: true`. | `destination`, `event_name`, `profile?` |
| `LogFailed` | A log primitive emission failed — unknown destination, sink error (HTTP non-2xx, connection refused), missing credential. Emitted fire-and-forget; never fails the run itself. `ok: false`. | `destination`, `event_name`, `profile?`, `reason` |
