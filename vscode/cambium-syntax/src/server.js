...
const PRIMITIVE_DOCS = {
  ...
  cron: {
    detail: 'Declares a scheduled fire (RED-273 / RED-305).',
    doc: '`cron` declares that this gen should run on a recurring schedule. Framework owns the declaration + runtime semantics (memory scope, fire_id, observability); operator\'s existing scheduler (crontab / k8s CronJob / Render / systemd / GH Actions) owns the lifecycle.\n\nNamed vocabulary:\n```ruby\ncron :daily, at: "9:00"\ncron :weekdays, at: "9:00"\ncron :weekly, at: "8:00"   # Sunday 8am\ncron :hourly\ncron :every_minute         # for testing\n```\n\nRaw crontab for anything the vocabulary doesn\'t cover:\n```ruby\ncron "30 14 * * 1-5"\n```\n\nKwargs: `at:` (HH:MM for daily/weekly/weekdays), `tz:` (default UTC), `method:` (which gen method to invoke), `id:` (explicit slug override).\n\nCompile to deploy-ready manifests with `cambium schedule compile <workspace> --target <k8s-cronjob|crontab|systemd|github-actions|render-cron>`.\n\nRuntime: operator invokes `cambium run <gen> --method <m> --fired-by schedule:<id>` at the declared cadence. `scope: :schedule` memory routes to a per-schedule bucket.',
  },
  log: {
