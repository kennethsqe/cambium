...
- **`repair`** (workspace slot, not a gen declaration): `app/config/models.rb` can name a cheaper model for repair passes — `repair "omlx:nemotron-3-nano-4b", max_tokens: 900`. Absent → repair runs on the gen's model (byte-identical IR); declared → it owns the structural repair sites only (`effort`/`fallbacks` are refused at parse time, not inherited from the gen). Semantic repair (review, consensus disagreement, corrector feedback, grounding) runs on the gen's model and is handed the task + source document, so a bad quote can be corrected instead of deleted — and a repair that deletes citations or grounded values fails the run (RED-175). See [`P - repair (model slot)`](docs/GenDSL%20Docs/P%20-%20repair%20%28model%20slot%29.md) for how to declare it, [`C - Repair Loop`](docs/GenDSL%20Docs/C%20-%20Repair%20Loop.md) § What repair sees, per failure class and [`N - Model Identifiers`](docs/GenDSL%20Docs/N%20-%20Model%20Identifiers.md) § The `repair` slot.
...
- **`grounded_in`**: citation enforcement with verbatim quote verification; `verify: :field_values` adds value-level cross-checks (RED-392). `format: :markdown | :json` (or inferred from the source path) makes the verifier match against a derived plain-text view as well as the raw text (#169). Enforcement is fail-closed end to end: a repair that deletes the citations fails the run instead of reporting `ok: true` with `totalChecked: 0` (RED-175). See [`P - grounded_in`](docs/GenDSL%20Docs/P%20-%20grounded_in.md).
- **`mode :agentic`**: multi-turn tool-use loop (model calls tools during generation); the shared head of the user prompt is cached per turn (#228)
- **`exclude_from_prefix`**: names context keys that reach the model but must NOT contribute to the cacheable prompt prefix (#182). The prefix is addressed by a content hash, so one per-call byte (`page_id` from a `fan_out` binding) splits the provider cache into one entry per branch; excluded keys render into the *uncached tail* of the user prompt instead — fully model-visible, just after the cache breakpoint — and the fan-out collapses to one prewarm group. **Scoped to the cached path**: below the ~4 KB floor, or on any provider that can't mark a breakpoint (oMLX/Ollama always), the declaration is a no-op and the prompt is byte-identical to a gen that never declared it — otherwise the legacy `<instruction>\n\n<prefix>` layout would reorder the prompt for zero benefit. Excluding a large key can itself drop the prefix under the floor and turn caching off; `Generate` meta carries `cache_prefix: { judged_chars, used, excluded_chars }` and the runner warns on that inversion. Naming the `grounded_in` source or a `_` key is a compile error; the IR field is omitted entirely when undeclared, so existing gens compile byte-identically. The `_` convention (hidden **and** excluded) is untouched. See [`P - GenModel`](docs/GenDSL%20Docs/P%20-%20GenModel.md) § `exclude_from_prefix`.
...
cambium run --ir <file.ir.json> [--method <m>] [--arg <path>|-] [--mock] ...   # execute a precompiled artifact, no Ruby (#195)
...
cambium serve --workspace <path> --bind <uri> [--precompiled|--ir-dir <dir>]   # long-lived HTTP server hosting every exported gen/pipeline (RED-360); precompiled boot needs no Ruby on PATH (#195)
...
- **Exact-duplicate tool calls are answered from memory, not re-dispatched (RED-184).** `handleAgenticGenerate` keeps a per-run `Map<fnName + args → result>`; a verbatim repeat is answered from the map and never reaches `handleToolCall` — no impl resolution, no AJV, no `buildToolContext`, no egress (nothing dispatches, so nothing can bypass the guard). It still gets a `duplicate: true` tool message, and `totalToolCalls` + `budget.addToolCall` still increment: faster trigger for the budget backstop, never a replacement. **Only successful dispatches enter the map** — the `set` lives inside the `try`, after the result push. Memoizing a throw would permanently block retry-after-transient-failure and would file the replay as `ok: true` when the real call was `ok: false`; a deterministically-failing tool re-dispatches and stays bounded by `maxToolCalls`, as before RED-184. Exact match only — reordered keys and reworded args are false negatives by design, and two unparseable arg blobs collapse to one `fnName + "{}"` signature (same dead end, deliberately one entry). Ceiling: a duplicate `read_file` after a write returns the pre-write bytes. See [`P - mode`](docs/GenDSL%20Docs/P%20-%20mode.md) § Semantics and the `AgenticTurn` row of [`C - Trace`](docs/GenDSL%20Docs/C%20-%20Trace%20%28observability%29.md).
...
| Memory decl `name` / `scope` from the IR (#195) | same guard, at the bucket-path resolver (any IR origin) + fail-fast in the artifact reader | `memory/path.ts#resolveBucketPath`, `ir-artifact.ts#assertGenIr` |
| `returnSchemaId` lookup (#195) | own-property only (`__proto__` reads as not found) + identifier / reserved-name check on artifacts | `runner.ts` schema lookup, `ir-artifact.ts#assertGenIr` |
...
- **Model aliases resolve at compile time only (RED-237)**: the TS runner never sees a Symbol; don't add runtime alias resolution. The RED-176 `repair` slot is resolved in the same pass and is workspace-authoritative — no per-gen `repair:` — and it covers only the structural repair sites; semantic repair runs on the gen's model. When it is declared, the model that actually ran is what the ceiling belongs to — the trace and the operator-facing error must name it, and name whoever declared the number (`gen` or `repair slot`). [`N - Model Identifiers`](docs/GenDSL%20Docs/N%20-%20Model%20Identifiers.md), [`C - Repair Loop`](docs/GenDSL%20Docs/C%20-%20Repair%20Loop.md)
- **Semantic repair is not context-free (RED-175)**: the five semantic repair sites (Review, consensus disagreement, corrector feedback, grounding citations, grounding field-values) pass `{ documents, groundingTextByKey, task }` and build their prompt with the SAME `buildGenSystem` / `buildCacheablePrefix` the Generate step used — re-deriving that prefix moves the prompt-cache key and re-bills the document on every attempt. The two structural sites stay context-free and byte-identical. **Same assemblers, deliberately not the same *variant* (#232):** repair always builds through the non-agentic `buildGenSystem(ir, schema)` + default `includeOutputTemplate`, `mode :agentic` gens included — it dispatches through tools-less `generateText`, and Anthropic builds prefixes `tools` → `system` → `messages`, so an agentic Generate's entry is unreachable from here whatever the prompt says. Threading the mode in buys no hit and costs two: it would claim tools that aren't wired up, and strip the block `REPAIR RULES` names by hand. The re-verify is fail-closed: `GroundingCheckAfterRepair` / `GroundingFieldValueCheckAfterRepair` compare citation (or grounded-value) counts before and after the repair and `deleted_by_repair: true` flips the step to `ok: false` AND `finalOk = false` — an output that lost its provenance is never reported as grounded. Deleting the guard re-opens the silent-downgrade hole #175 was filed for. [`C - Repair Loop`](docs/GenDSL%20Docs/C%20-%20Repair%20Loop.md), [`P - grounded_in`](docs/GenDSL%20Docs/P%20-%20grounded_in.md)
...
- **Agentic prompt caching: three explicit breakpoints, never four (#228)**: `buildAnthropicMessagesRequest` gates on `opts.tools !== undefined` — the tools path — and on it emits `system` + `tools[last]` + user-prefix explicitly, plus a **top-level** `cache_control` (Anthropic's automatic breakpoint, which the API advances through the transcript itself). Anthropic's ceiling is four and the automatic one consumes a slot, so a fourth explicit marker is an HTTP 400 on every turn: the last-document marker is suppressed whenever the prefix marker fires on this path. `generateText` bodies are byte-identical to pre-#228 and keep marking documents (C-1 — prompt-cache keys are content hashes; a stray space invalidates every warm entry a deployed consumer holds) (one exception: a nullish step.prompt normalizes to "" on both dispatch sites — see PLAN § C-1 exception). The agentic prefix is size-gated only — no `grounded_in` requirement, because it is re-sent every turn — while the single-shot gate stays grounded-only (OQ-002). Both dispatch sites build through the SAME `buildGenSystem` / `buildCacheablePrefix`; `buildGenSystem(ir, schema, { agentic: true })` is a parameter, never a second assembler. Providers without `supportsPromptCacheControl` see the prefix folded back into the first user message (`<prompt>\n\n<prefix>`) and no `cachedPrefix` field — one gate for both paths, never a second policy. [`N - Model Identifiers`](docs/GenDSL%20Docs/N%20-%20Model%20Identifiers.md) § Anthropic prompt caching, [`P - mode`](docs/GenDSL%20Docs/P%20-%20mode.md) § Semantics
...
- **Precompiled artifacts are closed IRs; the artifact's location anchors discovery; explicit `appRoot`/`engineDir` win (#195)**: `cambium serve --precompiled|--ir-dir` and `cambium run --ir` refuse an IR that still needs Ruby at run time (a pipeline, an `enrich` gen, or a retro memory-write agent) rather than let it fail at first dispatch; a symbol-form gen (`returns :Symbol`) without a `[types].contracts` declaration is refused the same way, before any run starts. Both anchor tool/action/provider/log-sink/contracts/corrector discovery on the artifact's own on-disk location, never `ir.entry.source` (a build-machine path) — `runGenFromIr`'s `opts.appRoot` wins as the tier-1 workspace whenever `<appRoot>/Genfile.toml` exists, and `opts.engineDir` wins outright when set. [`N - Precompiled IR Distribution (#195)`](docs/GenDSL%20Docs/N%20-%20Precompiled%20IR%20Distribution%20%28%23195%29.md), [`N - App Mode vs Engine Mode (RED-220)`](docs/GenDSL%20Docs/N%20-%20App%20Mode%20vs%20Engine%20Mode%20%28RED-220%29.md)
...
- **Fan-out cache prewarm is automatic, byte-identical, and best-effort** — a `fan_out` with `concurrency > 1` and a shared grounded prefix fires one warm-up per distinct (model tier × prefix) before dispatch, via `buildGenSystem`/`buildCacheablePrefix` (the SAME assemblers `handleGenerate` uses — reuse them, never re-derive the prefix, or the cache key drifts). Outer "Off" gates (entire prewarm skipped): `concurrency 1` / single-branch / `--mock` / `prewarm false` / nothing-shared (every cacheable branch its own group — `meta.prewarm.skipped: "no-shared-prefix"`). Per-branch filter inside `prewarmFanOut`: ungrounded branches, branches whose cacheable prefix is below `MIN_CACHE_PREFIX_CHARS`, and `mode :agentic` sub-gens (#228 — prewarm sends no tools, and Anthropic builds prefixes `tools` → `system` → `messages`, so the entry could never be read) are excluded individually, always **before** `eligible++`; the prewarm still fires for remaining eligible branches. Nothing-shared is counted on *eligible* branches, not `branches.length` — excluded branches are not evidence of a shared prefix, and 5 branches with 3 excluded and 2 distinct groups still has nothing to warm (RED-183). Mixed fan-outs (some share, some don't) still warm their singletons — the gate is all-or-nothing. A warm-up failure never fails the fan-out. See [`N - Orchestration Layer`](docs/GenDSL%20Docs/N%20-%20Orchestration%20Layer.md) § Fan-out cache prewarm.
...
Cambium work is tracked as issues in the Forgejo repo `sbkeider/cambium` — canonical URL `https://source.deerlarch.net`, LAN API `http://nas-pi:3001` (anon read works: `GET /api/v1/repos/sbkeider/cambium/issues?state=open`). Linear (`A-411`) is retired; its `RED-<NNN>` IDs stay as-is inside existing docs, commits and `docs/GenDSL Docs/` — they are NOT Forgejo issue numbers (new tickets take the next free issue number). Branch naming: `RED-<NNN>/<short-slug>`. Commit subjects: `RED-<NNN>: <message>`.
