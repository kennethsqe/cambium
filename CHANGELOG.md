...
- **Anthropic user-prompt-prefix caching for grounded fan-out gens (PR #18, contrib by kennethsqe).** When a gen has `grounded_in` set and the shared user-prompt payload (DOCUMENT + non-primary context + `OUTPUT_JSON_TEMPLATE`) clears Anthropic's cache floor, `handleGenerate` splits the prompt into a cacheable prefix block and a per-call instruction; the Anthropic provider emits the prefix as a separate text block with `cache_control: ephemeral`. In a fan-out sharing the same grounded document, branch 1 writes the cache and branches 2..N read it — estimated 55–65% input-cost reduction on large fan-out shapes. Non-grounded gens, gens below the cache floor, and non-Anthropic providers receive the byte-identical single-string prompt as before (the runner flattens upstream for providers that don't declare `supportsPromptCacheControl`). The agentic `generateWithTools` path is intentionally unchanged. **Migration note (AUD-002):** grounded Anthropic gens whose shared payload clears `MIN_CACHE_PREFIX_CHARS` (~4 KB) now place the document/context block *before* the per-call instruction in the Anthropic user message (prefix-first ordering required by Anthropic's caching contract). This aligns with Anthropic's "put longform data before the query" guidance and is neutral-to-beneficial for output quality. Under `--mock` (and therefore in golden tests) output is unchanged — the runner flattens to the legacy instruction-first layout, so committed snapshots still pass. The reorder is visible **only against live Anthropic** above the floor and may shift model output for prompts tuned pre-0.8.1; if you pin live output, re-capture and commit the new baseline per `P - Golden Tests` §Step 4 (`cambium run … --mock` is *not* sufficient — these snapshots come from live runs).
